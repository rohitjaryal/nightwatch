...
var SENTINEL = ['Started org.openqa.jetty.jetty.Server', 'INFO - Selenium Server is up and running','INFO - The node is registered to the hub and ready to use'];
...
function SeleniumServer(settings, callback) {
  ...
  this.error_out = '';
  this.process = null;
  this.hub=this.settings.selenium.hub;
}

SeleniumServer.prototype.setCliArgs = function() {
  if(this.hub !=null && this.hub.indexOf('register')!=-1){
    console.log('Going to start selenium node @'+this.hub);
    this.cliOpts = [
